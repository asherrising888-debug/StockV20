...
class AlgoEngine:
    ...
    def sync_history():
        ...
        try:
            ...
            rename_map = {'date': '日期', 'close': '收盘', 'open': '开盘', 'high': '最高', 'low': '最低',
                          'volume': '成交量'}
        ...
        try:
            ...
            if snapshot and code in snapshot:
                ...
                if today not in df.index:
                    df.loc[today] = [real['open'], real['close'], real['high'], real['low'], real['volume']] + [0] * (
                                len(df.columns) - 5)
                else:
                    df.loc[today, ['open', 'close', 'high', 'low', 'volume']] = [real['open'], real['close'],
                                                                                 real['high'], real['low'],
                                                                                 real['volume']]
...
with st.sidebar:
    ...
    if st.button("🔄 同步最新数据 (周五必点)", type="primary"):
        AlgoEngine.sync_history(st.write)
...
if st.button("🚀 启动全流程诊断", use_container_width=True):
    ...
    if pos_input:
        for line in pos_input.split('\n'):
            ...
            if len(p) >= 3:
                try:
                    positions.append({'code': p[0].strip(), 'cost': float(p[1]), 'shares': int(p[2]),
                                      'high': float(p[3]) if len(p) > 3 else float(p[1])})
                except:
                    pass
    ...
    if positions:
        for p in positions:
            ...
            reason = None
            if pct <= PARAMS['STOP_LOSS']:
                reason = f"硬止损(亏{pct:.1%})"
            elif price < d['MA20'] and not d['MA20_Up']:
                reason = "趋势破坏"
